...
<body>
    ...
    <div class="container">
        ...
        <div class="info-card">
            ...
            <p><strong>ID Control:</strong> {{ $control->id_control }}</p>
            <p><strong>Fecha:</strong> {{ \Carbon\Carbon::parse($control->fecha_turno)->format('d/m/Y') }}</p>
            <p><strong>Vehículo:</strong> Móvil {{ $control->numero_interno }} ({{ $control->placa }})</p>
            <p><strong>Conductor:</strong> {{ $control->primer_nombre }} {{ $control->primer_apellido }}</p>
        </div>

        <div class="form-container">
            <form action="{{ route('operadora.cumplimiento-turnos', $control->id_control) }}" method="POST">
                @csrf
                @method('PUT') 
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="nombre_franja" id="nombre_franja" required 
                           value="{{ old('nombre_franja', $control->nombre_franja) }}">
                </div>
                ...
                <div class="form-group" style="display: flex; gap: 20px;">
                    <div style="flex: 1;">
                        ...
                        <input type="time" name="hora_inicio" id="hora_inicio" required 
                               value="{{ old('hora_inicio', substr($control->hora_inicio, 0, 5)) }}">
                    </div>
                    <div style="flex: 1;">
                        <label for="hora_fin">Hora Fin (Franja)</label>
                        <input type="time" name="hora_fin" id="hora_fin" required 
                               value="{{ old('hora_fin', substr($control->hora_fin, 0, 5)) }}">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="checkbox-group">
                        <input type="checkbox" name="cruza_medianoche" id="cruza_medianoche" value="1" 
                               {{ old('cruza_medianoche', $control->cruza_medianoche) ? 'checked' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <input type="time" name="hora_llamado" id="hora_llamado" required 
                           value="{{ old('hora_llamado', substr($control->hora_llamado, 0, 5)) }}">
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="checkbox-group">
                        <input type="checkbox" name="respondio" id="respondio" value="1" 
                               {{ old('respondio', $control->respondio) ? 'checked' : '' }}>
                ...
                <div class="form-group">
                    <div class="checkbox-group">
                        <input type="checkbox" name="en_servicio" id="en_servicio" value="1" 
                               {{ old('en_servicio', $control->en_servicio) ? 'checked' : '' }}>
                ...
                <div class="form-actions">
                    ...
                    <a href="{{ route('operadora.cumplimiento-turnos') }}" class="btn btn-secondary">Volver</a>
                </div>
            </form>
            
            <form action="{{ route('operadora.cumplimiento-turnos', $control->id_control) }}" method="POST" style="margin-top: 15px;">
                @csrf
                @method('DELETE')
